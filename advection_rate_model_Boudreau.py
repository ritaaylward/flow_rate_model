'''calculate the rate of advection that best explains observed vaules of Li in sediment porewater'''
...
def find_Ds(node): # function to determine sediment diffusion coefficient at a given depth given porosity and Dm
    depth = ((node/n) * z_max) + 145 #145 is the depth of the mixed layer(it's already built in to c_jpc1 and z_jpc1)
    i = 0
    while por_arr[i][0] < depth: # find the i closest to depth we are looking at
        i += 1
    por = ((depth - por_arr[i-1][0]) / (por_arr[i][0] - por_arr[i-1][0]) * (por_arr[i][1] - por_arr[i-1][1])) + por_arr[i-1][1] # use that i to estimate porosity at that depth
    tort = 1 - np.log(por**2) # calculate tortuosity (Boudreau's law)
...
# if you take out the above content and just use a constant Ds, the model works.
    ...
    # Ds = (Dm / tort) * (3.154e11)
    # return Ds #return a constant Ds based on average porosity
...
# create matrix A
    ...
    for i in range(n+1):
        A[i,i] = s*(2*sigma(i)) + ((find_Ds(i+1) + 2*find_Ds(i) + find_Ds(i-1)) / D) # center diagonal

    for j in range(n):
# goes to n because there's always one less than on the center diagonal
        A[j+1, j] = -((s*(1+sigma(j))) + ((find_Ds(j-1) + find_Ds(j))/D)) # below center diagonal

        A[j, j+1] = (s*(1-sigma(j))) - ((find_Ds(j+1) + find_Ds(j))/D) # above center diagonal
...
# create matrix B
    ...
    B[0,0] = (( (s*(1+sigma(0))) + ((find_Ds(-1) + find_Ds(0))/ D)) * g1) + r[0] # overriding first value
    B[n,0] = (-( (s*(1-sigma(n-1))) - ((find_Ds(n-1) + find_Ds(n))/ D)) * g2) +r[n] # overriding the last value
...
def find_best_w(w_low, w_high, w_nodes): # Iterate through a specified range to find the advection rate w that best explains the observed data. Return that value for w and the associated concentration profile
    err_arr = [] #error array
    w_arr = [] #advection rate array
    c_arr = [] #modeled concentration array
    print("i   w   error\n-------------") # labels for iteration prints
    for i in range(w_nodes+1):
        w = w_low + ((w_high-w_low)*(i/w_nodes))
        if w == 0:  # get a divide by 0 error from the sigma function
            continue
        c = find_c(w)
        c_arr.append(c)
        c_error = []
        ...
        #print(c_arr)
    k = err_arr.index(min(err_arr)) #lowest error value
...
def plot_c(c, w): # function to plot results
    ...
    ax.set_title('Modeled [Li] for "Best" Advection Rate w (cm/yr)', fontsize = fs*1.2)
...
# execute code. find the best w (w with least error) between specified endpoints. print and plot findings
best_w, best_c, w_arr, c_arr = find_best_w(-1, 0,10)
